...
export default {
    async fetch(request, env) {
        ...
        if (pathname === '/api/files/exists' && method === 'POST') {
            return handleFileExists(request, GIT_REPO, GIT_TOKEN);
        }
...
async function handleFileExists(request, GIT_REPO, GIT_TOKEN) {
    try {
        const { path } = await request.json();
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body::-webkit-scrollbar-thumb { background: rgba(255, 144, 0, 0.6); border-radius: 4px; }
        .login-mask {
            position: fixed;
            top: 0;
            left: 0;
        ...
        .login-box h2 { color: #ff9000; margin-bottom: 30px; text-align: center; font-size: 24px; }
        .login-box input {
            width: 100%;
            padding: 12px 16px;
            margin: 10px 0;
        ...
        .login-box input:focus { outline: none; border-color: #ff9000; }
        .login-box button {
            width: 100%;
            padding: 12px;
            margin-top: 15px;
        ...
        .login-box button:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(255, 144, 0, 0.4); }
        #drop-zone { min-height: 100vh; display: flex; flex-direction: column; }
        .main-panel {
            flex: 1 0 auto;
            overflow: visible;
            display: flex;
        ...
        .main-panel::-webkit-scrollbar-track { background: rgba(255, 255, 255, 0.05); border-radius: 4px; }
        .main-panel::-webkit-scrollbar-thumb { background: rgba(255, 144, 0, 0.5); border-radius: 4px; }
        .main-panel::-webkit-scrollbar-thumb:hover { background: rgba(255, 144, 0, 0.8); }
        .top-bar {
            display: flex;
            flex-wrap: wrap;
            gap: 6px;
        ...
        .action-btn:hover { background: rgba(255, 144, 0, 0.2); transform: translateY(-1px); }
        .action-btn.primary {
            background: linear-gradient(135deg, #ff9000, #ffb347);
            color: #000;
            border: none;
        ...
        .action-btn.primary:hover { box-shadow: 0 4px 12px rgba(255, 144, 0, 0.4); }
        .cdn-section { margin-bottom: 12px; flex-shrink: 0; }
        .cdn-input-wrapper { margin-bottom: 10px; }
        .cdn-input-wrapper.hidden { display: none; }
        .cdn-input {
            width: 100%;
            padding: 10px 14px;
            background: rgba(255, 255, 255, 0.05);
        ...
        .cdn-input:focus { outline: none; border-color: #ff9000; }
        .cdn-buttons { display: flex; flex-wrap: wrap; gap: 8px; }
        .cdn-btn {
            padding: 6px 12px;
            background: rgba(255, 255, 255, 0.05);
            border: 1px solid rgba(255, 255, 255, 0.1);
        ...
        .cdn-btn:hover { border-color: #ff9000; color: #ff9000; }
        .cdn-btn.active { background: rgba(255, 144, 0, 0.2); border-color: #ff9000; color: #ff9000; }
        .speed-tag { font-size: 11px; padding: 2px 6px; border-radius: 4px; background: rgba(255, 255, 255, 0.1); }
        .speed-good { color: #4ade80; }
        .speed-slow { color: #f87171; }
        .file_wrap { flex: 1; overflow-y: visible; margin-top: 10px; }
        .file_list { list-style: none; display: flex; flex-direction: column; gap: 6px; }
        .file_list li {
            background: rgba(255, 255, 255, 0.03);
            border: 1px solid rgba(255, 255, 255, 0.05);
            border-radius: 8px;
        ...
        .file_list li:hover { background: rgba(255, 255, 255, 0.06); border-color: rgba(255, 144, 0, 0.3); }
        .file_list li:before { content: "📄"; font-size: 18px; min-width: 24px; }
        ...
        .file_list li.dir a { color: #ff9000; font-weight: 600; text-decoration: underline; }
        .file_list li span { color: #999; font-size: 12px; white-space: nowrap; }
        .file_list li span.sha { font-family: monospace; opacity: 0.6; }
        .file_list li span.size { min-width: 60px; text-align: right; }
        ...
        .file_list li .delete-btn:hover { background: rgba(248, 113, 113, 0.2); }
        ...
        .search-box input:focus { outline: none; border-color: #ff9000; width: 250px; }
        .drag-hint {
        ...
        #drop-zone.dragover { background: rgba(255, 144, 0, 0.15); border-radius: 12px; }
        .empty-folder-hint {
            text-align: center;
            padding: 60px 20px;
            border: 2px dashed rgba(255, 144, 0, 0.3);
        ...
        .progress-fill { height: 100%; background: linear-gradient(90deg, #ff9000, #ffb347); transition: width 0.3s ease; width: 0%; }
        ...
        .config-modal h3 { margin-bottom: 20px; color: #ff9000; font-size: 20px; display: flex; align-items: center; gap: 8px; }
        .config-modal label { display: block; margin: 15px 0 8px; color: #e0e0e0; font-size: 14px; font-weight: 500; }
        .config-modal input {
            width: 100%;
            padding: 12px 14px;
            background: rgba(255, 255, 255, 0.05);
        ...
        .config-modal input:focus { outline: none; border-color: #ff9000; background: rgba(255, 255, 255, 0.08); }
        .config-modal button {
            width: 100%;
            padding: 12px;
            margin-top: 20px;
        ...
        .config-modal button:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(255, 144, 0, 0.4); }
        .close-modal { background: rgba(255, 255, 255, 0.1) !important; color: #fff !important; margin-top: 10px; }
        .close-modal:hover { background: rgba(255, 255, 255, 0.15) !important; }
        .theme-switcher { display: flex; gap: 8px; align-items: center; }
        .theme-btn { width: 28px; height: 28px; border-radius: 50%; border: 2px solid rgba(255, 255, 255, 0.2); cursor: pointer; transition: all 0.2s; }
        .theme-btn:hover { transform: scale(1.1); border-color: #ff9000; }
        .theme-btn.active { border-color: #ff9000; box-shadow: 0 0 8px rgba(255, 144, 0, 0.5); }
        body.theme-light { background: linear-gradient(135deg, #f5f5f5 0%, #e8e8e8 100%); color: #1a1a1a; }
        body.theme-light .main-panel { background: rgba(255, 255, 255, 0.95); border-color: rgba(0, 0, 0, 0.15); }
        body.theme-light .file_list li { background: rgba(255, 255, 255, 0.9); border-color: rgba(0, 0, 0, 0.1); color: #1a1a1a; }
        body.theme-light .file_list li:hover { background: #ffffff; border-color: rgba(255, 144, 0, 0.5); }
        ...
        body.theme-light .file_list li .delete-btn { background: rgba(248, 113, 113, 0.15); border-color: rgba(248, 113, 113, 0.4); color: #dc2626; }
        body.theme-light .action-btn { background: rgba(255, 144, 0, 0.15); border-color: rgba(255, 144, 0, 0.4); color: #e67e00; }
        body.theme-light .cdn-btn { background: rgba(0, 0, 0, 0.05); border-color: rgba(0, 0, 0, 0.15); color: #333; }
        body.theme-light .cdn-input, body.theme-light .search-box input { background: rgba(255, 255, 255, 0.9); color: #1a1a1a; border-color: rgba(0, 0, 0, 0.2); }
        body.theme-light .drag-hint { background: rgba(255, 144, 0, 0.1); border-color: rgba(255, 144, 0, 0.4); color: #e67e00; }
        body.theme-light .empty-folder-hint { background: rgba(255, 144, 0, 0.05); border-color: rgba(255, 144, 0, 0.3); }
        body.theme-light .login-box { background: rgba(255, 255, 255, 0.95); border-color: rgba(255, 144, 0, 0.4); }
        body.theme-light .login-box input { background: rgba(0, 0, 0, 0.05); border-color: rgba(0, 0, 0, 0.15); color: #1a1a1a; }
        body.theme-blue { background: linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%); color: #e2e8f0; }
        body.theme-blue .main-panel { background: rgba(15, 23, 42, 0.8); border-color: rgba(59, 130, 246, 0.2); }
        body.theme-blue .file_list li { background: rgba(30, 41, 59, 0.6); border-color: rgba(59, 130, 246, 0.15); color: #e2e8f0; }
        body.theme-blue .action-btn { background: rgba(59, 130, 246, 0.15); border-color: rgba(59, 130, 246, 0.4); color: #60a5fa; }
        body.theme-blue .login-box { background: rgba(15, 23, 42, 0.9); border-color: rgba(59, 130, 246, 0.4); }
        body.theme-blue .login-box h2 { color: #60a5fa; }
        body.theme-blue .login-box button { background: linear-gradient(135deg, #3b82f6, #60a5fa); }
        body.theme-green { background: linear-gradient(135deg, #052e16 0%, #166534 100%); color: #ecfdf5; }
        body.theme-green .main-panel { background: rgba(5, 46, 22, 0.8); border-color: rgba(34, 197, 94, 0.2); }
        body.theme-green .action-btn { background: rgba(34, 197, 94, 0.15); border-color: rgba(34, 197, 94, 0.4); color: #4ade80; }
        body.theme-green .login-box { background: rgba(5, 46, 22, 0.9); border-color: rgba(34, 197, 94, 0.4); }
        body.theme-green .login-box h2 { color: #4ade80; }
        body.theme-green .login-box button { background: linear-gradient(135deg, #22c55e, #4ade80); }
        body.theme-purple { background: linear-gradient(135deg, #2e1065 0%, #6b21a8 100%); color: #faf5ff; }
        body.theme-purple .main-panel { background: rgba(46, 16, 101, 0.8); border-color: rgba(168, 85, 247, 0.2); }
        body.theme-purple .action-btn { background: rgba(168, 85, 247, 0.15); border-color: rgba(168, 85, 247, 0.4); color: #c084fc; }
        body.theme-purple .login-box { background: rgba(46, 16, 101, 0.9); border-color: rgba(168, 85, 247, 0.4); }
        body.theme-purple .login-box h2 { color: #c084fc; }
        body.theme-purple .login-box button { background: linear-gradient(135deg, #a855f7, #c084fc); }
...
<script>
    ...
    async function verifyPassword() {
        return new Promise((resolve) => {
            ...
            modalDiv.innerHTML = '<div class="config-modal">' +
                '<h3>🔐 身份验证</h3>' +
    ...
    async function uploadFile(path, file) {
        ...
        try {
            ...
            if (result.exists) {
                if (!confirm(\`文件已存在: \${path}\\n是否覆盖?\`)) {
                    return;
                }
                formData.append('sha', result.sha);
            }
        } catch (e) {
            // 检查失败，继续上传
        }
