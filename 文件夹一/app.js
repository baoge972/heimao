...
function titleOf(item) { return item?.title || item?.name || '未命名'; }
...
els.loadMoreButton.classList.remove('hidden');
els.loadMoreButton.textContent = `显示更多（${visibleResultCount}/${currentResults.length}）`;
els.loadMoreButton.style.display = 'block';
if (loadMoreBtn) {
...
function renderHome(sections) {
  ...
  requestAnimationFrame(() => {
    setTimeout(() => {
      const heroContainer = els.hero;
      const heroItems = document.querySelectorAll('.catalog-section .media-card');
      
      console.log('🔍 箭头创建检测 - 卡片数量:', heroItems.length);
      
      // 删除旧箭头
      document.querySelectorAll('.hero-arrow').forEach(el => el.remove());
      
      if (heroItems.length <= 1) {
        console.log('⚠️ 卡片数量不足');
        return;
      }
      ...
      leftArrow.setAttribute('aria-label', '上一部');
      
      const rightArrow = document.createElement('button');
      rightArrow.innerHTML = '›';
      rightArrow.className = 'hero-arrow hero-arrow-right';
      rightArrow.setAttribute('aria-label', '下一部');
      
      heroContainer.appendChild(leftArrow);
      heroContainer.appendChild(rightArrow);
      console.log('✅ 箭头已创建');
      
      const updateHero = (index) => {
        const cards = document.querySelectorAll('.catalog-section .media-card');
        if (!cards.length) return;
        heroIndex = (index + cards.length) % cards.length;
        const card = cards[heroIndex];
        const title = card.querySelector('.card-overlay strong')?.textContent || '未命名';
        const img = card.querySelector('img');
        const poster = img ? img.src : '';
        
        document.querySelector('#heroTitle').textContent = title;
        if (img) {
          document.querySelector('#heroArtwork').src = poster;
          document.querySelector('#heroArtwork').style.display = 'block';
        }
        const backdrop = document.querySelector('#heroBackdrop');
        if (backdrop && poster) {
      ...
      leftArrow.addEventListener('click', () => updateHero(heroIndex - 1));
      rightArrow.addEventListener('click', () => updateHero(heroIndex + 1));
