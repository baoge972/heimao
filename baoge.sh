...
echo "----------------------------------------"
df -h | grep -E "Filesystem|/dev/vd|/dev/sd|/dev/sda" | sed 's/^/  /'
echo ""

# -------------------- 端口占用 --------------------
echo -e "${BLUE}【5】端口占用情况${NC}"
echo "----------------------------------------"
PORT_LIST=$(ss -tlnp 2>/dev/null | grep LISTEN | awk '{print $4}' | sed 's/.*://' | sort -n | uniq)
PORT_COUNT=$(echo "$PORT_LIST" | grep -v '^$' | wc -l)
echo "  已占用端口总数: $PORT_COUNT"
echo "  端口列表: $(echo "$PORT_LIST" | tr '\n' ' ')"
echo ""
echo "  详细监听信息:"
...
if command -v docker &> /dev/null; then
    DOCKER_COUNT=$(docker ps -a --format "table {{.Names}}" 2>/dev/null | grep -v NAMES | grep -v '^$' | wc -l)
    echo "  Docker 容器总数: $DOCKER_COUNT"
...
if command -v docker &> /dev/null; then
    for container in $(docker ps -a --format "{{.Names}}" 2>/dev/null); do
        ...
        PORTS=$(docker port $container 2>/dev/null | awk '{print $3}' | tr '\n' ' ')
        STATUS=$(docker inspect --format='{{.State.Status}}' $container 2>/dev/null)
        echo "  $PROJECT_COUNT. $container"
        echo "     部署方式: Docker"
        echo "     端口: $PORTS"
        echo "     状态: $STATUS"
        echo ""
...
if ps aux | grep -v grep | grep -q "python3 -m http.server"; then
    ...
    echo "  $PROJECT_COUNT. Python HTTP Server"
    echo "     部署方式: 手动 (Python)"
    echo "     端口: $PORT"
    echo "     状态: 运行中"
    echo ""
...
if ps aux | grep -v grep | grep -q "node.*server.js"; then
    PROJECT_COUNT=$((PROJECT_COUNT + 1))
    echo "  $PROJECT_COUNT. Node.js 后端服务"
...
if ps aux | grep -v grep | grep -q "uvicorn"; then
    ...
    echo "  $PROJECT_COUNT. Uvicorn API 服务"
...
# 新增检测：Docker 容器内的 Nginx（科技lion部署）
if command -v docker &> /dev/null && docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^nginx$'; then
    NGINX_FOUND=1
    echo "  [Docker容器] Nginx 状态: 运行中"
    echo "  配置文件:"
    for conf in $(docker exec nginx ls /etc/nginx/conf.d/*.conf 2>/dev/null); do
        DOMAIN=$(docker exec nginx cat "$conf" 2>/dev/null | grep -E "^\s*server_name\s+" | head -1 | sed 's/.*server_name//' | sed 's/;//' | xargs)
        HAS_SSL=$(docker exec nginx grep -c "listen 443" "$conf" 2>/dev/null || echo 0)
        HAS_PROXY=$(docker exec nginx grep -c "proxy_pass" "$conf" 2>/dev/null || echo 0)
        if [ -n "$DOMAIN" ] && [ "$DOMAIN" != "_" ]; then
            SSL_STATUS=$([ $HAS_SSL -gt 0 ] && echo "✅ HTTPS" || echo "❌ HTTP")
            PROXY_STATUS=$([ $HAS_PROXY -gt 0 ] && echo "反向代理" || echo "静态文件")
            echo "    - $DOMAIN ($PROXY_STATUS, $SSL_STATUS)"
